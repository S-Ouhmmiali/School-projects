...
(*Type pour les types nommés globales *)
type typeNommeGlobal = Typenomme of string * typ

type structt = StRuct of (typ * string) list
(* Type affectable *)
type affectable  = 
       | Variable of string
       | Deref of affectable
...
type expression =
  ...
  | Initilisation of typ
  (* Valeur d'un affectable *)
  | Val of affectable
  (* Addresse d'une variable*)
  | Addresse of string
...
and instruction =
  ...
  (* Assignation d'addistion d'un affectable *)
  | Assignation of affectable * expression
  ...
  (* Declaration de type nommé local*)
...
type programme = Programme of typeNommeGlobal list * fonction list * bloc
...
struct
  (*Type nommé global*)
  type typeNommeGlobal = Typenomme of Tds.info_ast * typ

  type structt = StRuct of (typ * Tds.info_ast) list

  (* Type affectable *)
type affectable  = 
  | Variable of Tds.info_ast
  | Deref of affectable
  ...
  type bloc = instruction list
  and instruction =
    | Declaration of Tds.info_ast list * typ * Tds.info_ast * expression (* le nom de l'identifiant est remplacé par ses informations *)
    | Affectation of  affectable * expression 
    | Assignation of affectable * expression
    | Affichage of expression
    | Conditionnelle of expression * bloc * bloc
...
type bloc = instruction list
 and instruction =
  | Declaration of Tds.info_ast list * Tds.info_ast * expression
  | Affectation of affectable * expression
  | Assignation of affectable * expression
