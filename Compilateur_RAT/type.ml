type typ = Bool | Int | Rat |TypeNomme of string| Pointeur of typ | Struct of (typ * string) list| Undefined
...
let rec string_of_type t = 
  ...
  | Int  ->  "Int"
  | Rat  ->  "Rat"
  | TypeNomme(n) -> "TypeNommé"^n
  | Undefined -> "Undefined"
  | Pointeur(t) -> "Pointeur "^(string_of_type t)
  | Struct(l) -> "Struct{"^ (List.fold_left (fun x y ->  x^string_of_type (fst y)) "" l) ^ "}"
...
let  rec est_compatible t1 t2 =
  ...
  | Int, Int -> true
  | Rat, Rat -> true 
  | TypeNomme(_), TypeNomme(_) -> true
  | Pointeur(t),Pointeur(t') -> est_compatible t t'  (* t/t' peut etre un pointeur ou pointeur sur pointeur etc *)
  | Struct(l),Struct(l') ->  let l1,_ = List.split l in 
                             let l2,_ = List.split l' in 
                             List.for_all2 est_compatible l1 l2
  | _ -> false 
